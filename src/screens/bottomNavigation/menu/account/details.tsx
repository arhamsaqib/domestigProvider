...
const ShowImage = (props: PropsShowImage) => {
  return (
    <Modal visible={props.visible}>
      <SafeAreaView style={styles.modal}>
        <Image
          source={{uri: MEDIA_URL + props.source}}
...
export const ProfileDetails = () => {
  ...
  const [imageToOpen, setImageToOpen]: any = useState([]);
  ...
  function renderGallery({item}: any) {
    return (
      <TouchableOpacity
        onPress={() => {
          setImageToOpen(item.image), setShowModal(true);
        }}
        style={{
          width: '32%',
          marginRight: '2%',
